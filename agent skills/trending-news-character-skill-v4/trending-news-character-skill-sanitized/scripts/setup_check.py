...
def check_env(video: dict[str, Any], research: dict[str, Any], errors: list[str]) -> None:
    ...
    if voice_provider == "elevenlabs" and is_placeholder(os.environ.get("ELEVENLABS_API_KEY")):
...
def check_video(video: dict[str, Any], errors: list[str]) -> None:
    ...
    if video.get("captions_enabled") is not True:
        errors.append("config/video.json captions_enabled must be true for the current captioned workflow")
    captions = video.get("captions", {})
    if captions.get("font_name") != "JetBrains Mono":
        errors.append("config/video.json captions.font_name must be JetBrains Mono")
    if captions.get("caption_zone") != "above-ticker":
        errors.append("config/video.json captions.caption_zone must be above-ticker")
    if captions.get("mode") != "three-word-highlight":
        errors.append("config/video.json captions.mode must be three-word-highlight")
    if captions.get("animation") != "none":
        errors.append("config/video.json captions.animation must be none")
    if captions.get("active_word_color") != "#ff003c":
        errors.append("config/video.json captions.active_word_color must be #ff003c")
    safe_zone = layout.get("hyperframes_safe_zone", {})
    if safe_zone.get("height") != 890 or safe_zone.get("bottom_y") != 925:
        errors.append("config/video.json layout.hyperframes_safe_zone must keep the centered 960x890 safe zone")
    if safe_zone.get("source_css") != "left:60px; top:95px; width:960px; height:890px":
        errors.append("config/video.json layout.hyperframes_safe_zone.source_css must match the restored square-source safe-zone coordinates")
    if "caption_band" in layout:
        errors.append("config/video.json layout must not shrink HyperFrames with a caption_band; captions are post-assembly overlays above the ticker")
    if captions.get("transcription_provider") not in {"auto", "elevenlabs", "hyperframes"}:
        errors.append("config/video.json captions.transcription_provider must be auto, elevenlabs, or hyperframes")
    caption_font = ROOT / str(captions.get("font_path", ""))
    if not caption_font.exists():
        errors.append(f"caption font does not exist: {caption_font}")
