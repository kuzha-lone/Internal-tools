...
def default_safe_zone() -> Dict[str, object]:
    return {
        ...
        "edge_exclusion_rule": "All major content must stay inside the 960x890 primary animation safe zone within the 1080x960 top-half viewport; only background texture, glows, and non-essential decoratives may live outside it.",
...
def build_master_prompt(topic: str, script: str) -> str:
    ...
    return (
        ...
        "The visible layout truth is the centered 960x890 safe zone inside the 1080x960 top-half viewport; when authoring the square 1080x1080 source, use left:60px; top:95px; width:960px; height:890px for the safe-zone container. "
        "Primary scene content must be center-weighted and dense inside that zone; never leave the middle empty, pin the whole beat to the top, or use giant hollow panels. "
