...
description: Produce subject-specific, broadcast-quality HyperFrames visuals for the top half of a 1080x1920 split-screen short, keeping primary content inside the centered 960x890 primary animation safe zone with the ticker glued to the bottom of the top-half viewport while preserving strong art direction, narration timing, and editorial clarity.
---
...
- Primary animation safe zone: centered `960x890` inside the top half at x=60..1020 and y=35..925
- Square source CSS equivalent: `left: 60px; top: 95px; width: 960px; height: 890px;` because the 1080x1080 HyperFrames source is cropped to the visible 1080x960 top viewport
- Ticker band: y=925..960 in the final top-half viewport, glued to the bottom edge directly above the avatar split
...
5. **Layout Before Motion**
   Build each scene at its fully readable hero frame first. Use flex/grid containers and stable dimensions. The scene must be center-weighted inside the safe zone, with the dominant board/chart/map/photo/stat cluster occupying the middle of the frame. Add GSAP entrances/exits only after the static layout fits.
...
- Keep the hero frame visually filled inside the centered safe zone. Top-heavy layouts, bottom-only layouts, and hollow-center layouts are invalid.
...
- Do not leave the middle of the top half empty. Avoid top-anchored headline-only layouts, giant empty panels, and decorative boxes that do not carry information.
- Prefer dense editorial boards: compact headlines plus meaningful charts, stat cards, maps, source cards, comparison boards, or screenshot/photo panels that visibly fill the centered safe zone.
