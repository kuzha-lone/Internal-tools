#!/usr/bin/env python3
"""Validate short trending-news scripts against the configured character template rules."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate a 15-30 second configured-character trending-news script.")
...
def validate_script(script: str, min_chars: int, max_chars: int, min_words: int, max_words: int) -> dict:
    ...
    if not normalized.startswith(OPENING_PATTERNS):
        violations.append("Script must open with a configured news-character framing line.")
