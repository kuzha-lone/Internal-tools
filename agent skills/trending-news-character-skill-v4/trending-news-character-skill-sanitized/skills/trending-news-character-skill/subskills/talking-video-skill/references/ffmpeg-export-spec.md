...
- timed JetBrains Mono 3-word highlighted captions in neon cyberpunk red above the HyperFrames ticker, added after assembly without shrinking the centered 960x890 HyperFrames safe zone
...
- after the title exists, burn timed captions with `heygen_video.py caption-final` using JetBrains Mono, the `above-ticker` caption zone, `three-word-highlight` caption mode, and default animation `none`
- after captions exist, speed up the fully assembled captioned master to 1.5x
...
- captions are enabled for the current workflow
- the current priority is clean top/bottom composition, readable title, readable timed captions, and fast final export
