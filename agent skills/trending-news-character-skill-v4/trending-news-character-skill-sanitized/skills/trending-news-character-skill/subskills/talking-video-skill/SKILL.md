...
python3 {baseDir}/scripts/heygen_video.py caption-final --video-file <final-master> --script-file <approved-script> --output-dir <draft-dir> --basename <basename>
ffmpeg -y -i <captioned-master> -filter_complex "[0:v]setpts=PTS/1.5[v];[0:a]atempo=1.5[a]" -map "[v]" -map "[a]" -c:v libx264 -pix_fmt yuv420p -c:a aac <final-master-1p5x>
```
...
- after the HyperFrames top-half clip and assembled final master both exist, burn timed captions into the assembled master, then speed up the captioned final video to 1.5x; do not speed up the talking-head clip by itself
...
- add captions with `heygen_video.py caption-final` after `assemble_vertical_video.py --title` and before the 1.5x speed pass
- use `JetBrainsMono-Bold.ttf` / `JetBrains Mono` for captions by default
- use stable 3-word caption cue groups with the active spoken word highlighted in neon cyberpunk red (`#ff003c`); default caption animation is `none` so the text does not pop, scale, or bounce
- use `--transcription-provider auto` by default; it uses ElevenLabs if configured and falls back to HyperFrames/Whisper when ElevenLabs is not configured
- keep captions in the top-half empty space just above the HyperFrames ticker so they do not collide with the face, title band, HyperFrames main content, or ticker
...
- after assembly and speed pass, extract a few frames or a contact sheet from the final output and visually verify that the lower-third title is readable in the bottom talking-head section and the timed captions are readable above the HyperFrames ticker
