...
DEFAULT_FONT_PATH = ASSET_DIR / "fonts" / "JetBrainsMono-Bold.ttf"
...
DEFAULT_FONT_NAME = "JetBrains Mono"
DEFAULT_ELEVENLABS_TTS_MODEL = "eleven_multilingual_v2"
DEFAULT_ELEVENLABS_OUTPUT_FORMAT = "mp3_44100_128"
DEFAULT_CAPTION_ZONE = "above-ticker"
DEFAULT_CAPTION_ANIMATION = "none"
DEFAULT_CAPTION_MODE = "three-word-highlight"
DEFAULT_ACTIVE_WORD_COLOR = "#ff003c"
...
def parse_args() -> argparse.Namespace:
    ...
    render.add_argument("--transcription-provider", choices=("auto", "elevenlabs", "hyperframes"), default="auto", help="Word timestamp provider for optional caption finalization.")
    render.add_argument("--hyperframes-model", default="small.en", help="HyperFrames/Whisper model for caption transcription fallback.")
    ...
    render.add_argument("--caption-zone", choices=("upper-third", "lower-third", "above-title", "above-ticker"), default=DEFAULT_CAPTION_ZONE)
    render.add_argument("--caption-animation", choices=("pop-fade", "fade", "none"), default=DEFAULT_CAPTION_ANIMATION)
    render.add_argument("--caption-mode", choices=("standard", "three-word-highlight"), default=DEFAULT_CAPTION_MODE)
    render.add_argument("--active-word-color", default=DEFAULT_ACTIVE_WORD_COLOR)
    render.add_argument("--inactive-word-color", default=DEFAULT_INACTIVE_WORD_COLOR)
    ...
    finalize.add_argument("--transcription-provider", choices=("auto", "elevenlabs", "hyperframes"), default="auto", help="Word timestamp provider for caption transcription.")
    finalize.add_argument("--hyperframes-model", default="small.en", help="HyperFrames/Whisper model for caption transcription fallback.")
    ...
    finalize.add_argument("--caption-zone", choices=("upper-third", "lower-third", "above-title", "above-ticker"), default=DEFAULT_CAPTION_ZONE)
    finalize.add_argument("--caption-animation", choices=("pop-fade", "fade", "none"), default=DEFAULT_CAPTION_ANIMATION)
    finalize.add_argument("--caption-mode", choices=("standard", "three-word-highlight"), default=DEFAULT_CAPTION_MODE)
    finalize.add_argument("--active-word-color", default=DEFAULT_ACTIVE_WORD_COLOR)
    finalize.add_argument("--inactive-word-color", default=DEFAULT_INACTIVE_WORD_COLOR)
    finalize.add_argument("--no-overlay-title", action="store_true", help="Do not add the legacy persistent ASS title while captioning.")

    caption_final = subparsers.add_parser("caption-final", help="Burn timed captions into the assembled 1080x1920 master before the final speed pass.")
    caption_final.add_argument("--video-file", required=True, help="Existing assembled 1080x1920 master path.")
    caption_final.add_argument("--script", help="Approved final script.")
    caption_final.add_argument("--script-file", help="Path to a file containing the approved final script.")
    caption_final.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_DIR), help="Directory for generated caption assets.")
    caption_final.add_argument("--basename", help="Optional basename for generated caption assets.")
    caption_final.add_argument("--font-file", default=str(DEFAULT_FONT_PATH))
    caption_final.add_argument("--font-name", default=DEFAULT_FONT_NAME, help="Caption font family override for ASS generation.")
    caption_final.add_argument("--scribe-model", default="scribe_v2", help="ElevenLabs transcription model to use for word timestamps.")
    caption_final.add_argument("--transcription-provider", choices=("auto", "elevenlabs", "hyperframes"), default="auto", help="Word timestamp provider for caption transcription.")
    caption_final.add_argument("--hyperframes-model", default="small.en", help="HyperFrames/Whisper model for caption transcription fallback.")
    caption_final.add_argument("--language-code", default="en", help="Optional ISO-639 language hint for ElevenLabs Scribe.")
    caption_final.add_argument("--transcript-json", help="Optional existing transcript JSON with word timestamps.")
    caption_final.add_argument("--keep-audio", action="store_true", help="Keep the extracted WAV after transcription.")
    caption_final.add_argument("--max-line-chars", type=int, default=18)
    caption_final.add_argument("--max-words-per-cue", type=int, default=7)
    caption_final.add_argument("--caption-zone", choices=("upper-third", "lower-third", "above-title", "above-ticker"), default=DEFAULT_CAPTION_ZONE)
    caption_final.add_argument("--caption-animation", choices=("pop-fade", "fade", "none"), default=DEFAULT_CAPTION_ANIMATION)
    caption_final.add_argument("--caption-mode", choices=("standard", "three-word-highlight"), default=DEFAULT_CAPTION_MODE)
    caption_final.add_argument("--active-word-color", default=DEFAULT_ACTIVE_WORD_COLOR)
    caption_final.add_argument("--inactive-word-color", default=DEFAULT_INACTIVE_WORD_COLOR)
...
def safe_stem(value: str) -> str:
    ...
    return stem or "trending-news-character"
...
def download_file(url: str, destination: pathlib.Path) -> pathlib.Path:
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "trending-news-character-asset-download"})
    try:
...
def resolve_render_voice(args: argparse.Namespace, script: str) -> tuple[Dict[str, Any], Dict[str, Any] | None]:
    ...
    temp_dir = pathlib.Path(
        ...
        or tempfile.mkdtemp(prefix="trending-news-character-elevenlabs-")
    )
    ensure_dir(temp_dir)
...
def parse_json_stdout(stdout: str) -> Dict[str, Any]:
    text = stdout.strip()
    if not text:
        raise RuntimeError("Transcription command did not return JSON.")
    try:
...
def resolve_hyperframes_transcript_payload(payload: Any, audio_path: pathlib.Path) -> Any:
    ...
    if not transcript_path.is_absolute():
        transcript_path = (audio_path.parent / transcript_path).resolve()
    if not transcript_path.exists():
        raise RuntimeError(f"HyperFrames transcript file was not found: {transcript_path}")
    with open(transcript_path, "r", encoding="utf-8") as handle:
        return json.load(handle)


def run_hyperframes_transcription(
    audio_path: pathlib.Path,
    *,
    model_id: str,
    language_code: str | None,
    ...
    return {
        "backend": "hyperframes_whisper",
        "model": model_id,
        "result": resolve_hyperframes_transcript_payload(payload, audio_path),
    }
...
def load_transcription(
    *,
    ...
    if provider == "auto":
        provider = "elevenlabs" if not is_placeholder(os.environ.get("ELEVENLABS_API_KEY")) else "hyperframes"
    if provider == "elevenlabs":
        return run_elevenlabs_transcription(audio_path, model_id=model_id, language_code=language_code)
    if provider == "hyperframes":
        return run_hyperframes_transcription(audio_path, model_id=hyperframes_model, language_code=language_code)
...
def extract_timed_words(payload: Dict[str, Any] | List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    words: List[Dict[str, Any]] = []
    if isinstance(payload, list):
        iterable = payload
        for item in iterable or []:
            word_text = str(item.get("text") or item.get("word") or "").strip()
...
def chunk_three_word_highlight_tokens(tokens: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    cues: List[Dict[str, Any]] = []
    for group_start in range(0, len(tokens), 3):
        group = tokens[group_start : group_start + 3]
        if not group:
            continue
        group_end = float(group[-1]["end"] + 0.08)
        ...
        for active_index, token in enumerate(group):
            ...
            end = float(max(start + 0.08, next_start))
            cues.append(
                {
                    "start": start,
                    "end": end,
...
def normalize_highlight_cues(cues: List[Dict[str, Any]], duration: float) -> List[Dict[str, Any]]:
    normalized = [dict(cue) for cue in cues]
    for idx, cue in enumerate(normalized):
        cue["start"] = max(0.0, float(cue["start"]))
        cue["end"] = min(duration, max(cue["start"] + 0.08, float(cue["end"])))
        if idx > 0 and cue["start"] < normalized[idx - 1]["end"]:
            cue["start"] = normalized[idx - 1]["end"]
            cue["end"] = max(cue["start"] + 0.08, cue["end"])
    if normalized:
        normalized[-1]["end"] = min(duration, normalized[-1]["end"])
    return normalized
...
def caption_animation_prefix(animation_style: str) -> str:
    if animation_style == "none":
        return ""
    if animation_style == "fade":
        return r"{\fad(80,100)}"
    if animation_style == "pop-fade":
        return r"{\fad(70,90)\fscx92\fscy92\t(0,120,\fscx106\fscy106)\t(120,230,\fscx100\fscy100)}"
    raise RuntimeError(f"Unsupported caption animation style: {animation_style}")


def ass_color_from_hex(value: str) -> str:
    cleaned = value.strip().lstrip("#")
    if len(cleaned) != 6 or not re.fullmatch(r"[0-9a-fA-F]{6}", cleaned):
        raise RuntimeError(f"Invalid caption color: {value}")
    red = cleaned[0:2]
    green = cleaned[2:4]
    blue = cleaned[4:6]
...
def write_ass_subtitles(
    ...
    *,
    ...
    if caption_zone == "upper-third":
        ...
        alignment = 2
        margin_v = 210
    elif caption_zone == "above-title":
        alignment = 2
        margin_v = 520
    elif caption_zone == "above-ticker":
        alignment = 2
        margin_v = 1040
    else:
        raise RuntimeError(f"Unsupported caption zone: {caption_zone}")
    caption_prefix = caption_animation_prefix(caption_animation)
    ...
    header = [
        ...
        (
            "Style: Default,"
            f"{font_name},50,&H00FFFFFF,&H000000FF,&H00000000,&H78000000,"
            f"1,0,0,0,100,100,0,0,1,4.6,1.4,{alignment},96,96,{margin_v},1"
        ),
        (
            "Style: Title,"
    ...
    for cue in cues:
        if caption_mode == "three-word-highlight":
            ...
            text = caption_prefix + r"\N".join(escape_ass_text(line) for line in cue["lines"])
        event_lines.append(
            "Dialogue: 0,"
...
def finalize_caption_pipeline(
    *,
    ...
    transcription = load_transcription(
        ...
        hyperframes_model=hyperframes_model,
    ...
    if caption_mode == "three-word-highlight":
        cues = chunk_three_word_highlight_tokens(filled_tokens)
        normalized_cues = normalize_highlight_cues(cues, duration)
    else:
            ...
            if state == "completed" and normalized["video_url"] and (args.download_dir or args.finalize_output_dir):
                ...
                if args.finalize_output_dir:
                    result["caption_package"] = finalize_caption_pipeline(
                        ...
                        hyperframes_model=args.hyperframes_model,
                        language_code=args.language_code,
...
def run_finalize(args: argparse.Namespace) -> int:
    script = load_script(args.script, args.script_file)
    if args.no_overlay_title:
        overlay_title, overlay_title_source = None, "disabled"
    else:
        overlay_title, overlay_title_source = resolve_overlay_title(args.title, args.title_file, script)
...
def run_caption_final(args: argparse.Namespace) -> int:
    script = load_script(args.script, args.script_file)
    output_dir = pathlib.Path(args.output_dir)
    ensure_dir(output_dir)
    video_path = pathlib.Path(args.video_file).resolve()
    basename = safe_stem(args.basename or f"{video_path.stem}-captioned")
    result = finalize_caption_pipeline(
        ...
        inactive_word_color=args.inactive_word_color,
        overlay_title=None,
        overlay_title_source="already-burned-lower-third",
    )
    print(json.dumps(result, indent=2))
    return 0
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "caption-final":
            return run_caption_final(args)
